...
import requests
import time
kit = pnwkit.QueryKit("ef52dec875b199")
aaid = 11880
useridallowed = [588667852190515201,963674206841757736,841475930747830273,873973179557752862]
...
async def checkmmr(ctx, arg=None):
	...
	if not matchh:
		...
		return
	query = kit.query("nations", {"alliance_id": aaid, "first": 500}, "nation_name", "discord", "num_cities", pnwkit.Field("cities",{},"barracks","factory","hangar","drydock"))
...
async def checkwarchest(ctx):
	...
	if not matchh:
		await ctx.send("You are not authorised to execute this command. Contact '@cobias.' to get whitelisted.")
		return
	query = kit.query("nations", {"alliance_id": aaid, "first": 500}, "nation_name", "discord", "num_cities", "aluminum", "steel", "gasoline", "munitions","money","uranium","food")
	await ctx.send("Fetching, please wait... (approx. 5 secs)")
	result = query.get()
	time.sleep(5)
	passs = False
...
async def checkwarchestlegacy(ctx,arg1,arg2,arg3,arg4,arg5,arg6,arg7):
	...
	if not matchh:
		await ctx.send("You are not authorised to execute this command. Contact '@cobias.' to get whitelisted.")
		return
	query = kit.query("nations", {"alliance_id": aaid, "first": 500}, "nation_name", "discord", "num_cities", "aluminum", "steel", "gasoline", "munitions","food","money","uranium")
	await ctx.send("Fetching, please wait... (approx. 5 secs)")
	result = query.get()
	time.sleep(5)
	passs = False
...
async def checkspies(ctx):
	...
	if not matchh:
		await ctx.send("You are not authorised to execute this command. Contact '@cobias.' to get whitelisted.")
		return
	query = kit.query("nations", {"alliance_id": aaid, "first": 500}, "nation_name", "discord", "spies", "central_intelligence_agency")
	await ctx.send("Fetching, please wait... (approx. 5 secs)")
	result = query.get()
	time.sleep(5)
	final = ""
	...
	for x in range(len(result.nations)):
		CIA = ""
		total += 1
		if result.nations[x].central_intelligence_agency == True:
			CIA = " - Has CIA"
...
client.run("MTE0NzU3NDk1MTc0NzAxMDc2Mw.GwIIZ9.nizaebjTK89Tmiyjj2-XTIp_xQsTWwFZOfkqTM")
